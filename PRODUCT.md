...
`pdf.js`, lazy-loaded chunk), SVG → PNG/JPEG (canvas, shared `svgToRaster`). PlantUML viewer (in-browser `@plantuml/core`, lazy chunk; SVG + PNG per diagram). Mermaid viewer (in-browser `mermaid`, lazy chunk; SVG + PNG per diagram).
...
- Heavy dependencies (`pdf.js`, `jszip`, `@plantuml/core`, `mermaid`) must stay in an async chunk isolated to the tool that needs them so first paint elsewhere stays small.
